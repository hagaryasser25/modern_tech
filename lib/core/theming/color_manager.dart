import 'package:flutter/material.dart';

const Color backgroundColor =Color(0xFFFFFFFF);
const Color titleColor =Color(0xFF111827);
const Color subtitleColor =Color(0xFF6C7278);
const Color borderColor =Color(0xFFEDF1F3);
const Color primaryColor =Color(0xFF123CBF);
const Color disableColor =Color(0xFF648DDB);
const Color lightBlue =Color(0xFF4D81E7);
const Color darkBlue =Color(0xFF1E3A8A);
const Color googleButton =Color(0xFF1A1C1E);
const Color greyColor =Color(0xFF989898);
const Color lightGreyColor =Color(0xFFB3B3B3);
const Color greyText =Color(0xFF8F9BB3);
const Color blackColor =Color(0xFF000000);
const Color greyTitle =Color(0xFFADA4A5);
const Color semiDarkGrey =Color(0xFF6C7072);
const Color listColor =Color(0xFF0F0F0F);
const Color redColor =Color(0xFFCE2727);
const Color darkGrey =Color.fromARGB(255, 97, 102, 116);
const Color lightBlack =Color(0xFF0A1215);











